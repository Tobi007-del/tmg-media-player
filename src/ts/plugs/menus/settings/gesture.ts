            ...
            { id: "gestureDblClick", label: "Double click", widget: "select", getValue: () => (plug.config.dblClick === false ? "None" : capitalize(uncamelize(plug.config.dblClick))), getOptions: () => getClickOptions(plug) as UITuple<string>[], onChange: (val: any) => (plug.config.dblClick = val), configPaths: ["settings.gesture.dblClick"], getTipHTML: () => "Action to perform when double tapping on the video player" },
            {
              ...
              items: [
                ...
                { id: "gestureTouchTimeline", label: "Seek swipe", widget: "toggle", getValue: () => (plug.config.touch.timeline ? "On" : "Off"), onChange: (val: boolean) => (plug.config.touch.timeline = val), configPaths: ["settings.gesture.touch.timeline"], title: "Swipe left or right anywhere on the screen to seek" },
                {
                  ...
                  items: [
                    { id: "gestureTouchXRatio", label: "Horizontal", widget: "range", getValue: () => String(plug.config.touch.xRatio), getRange: () => ({ min: 0.1, max: 10, step: 0.1, formatTooltip: (v: number) => v.toFixed(1) }), onChange: (val: number) => (plug.config.touch.xRatio = val), configPaths: ["settings.gesture.touch.xRatio"] },
                    { id: "gestureTouchYRatio", label: "Vertical", widget: "range", getValue: () => String(plug.config.touch.yRatio), getRange: () => ({ min: 0.1, max: 10, step: 0.1, formatTooltip: (v: number) => v.toFixed(1) }), onChange: (val: number) => (plug.config.touch.yRatio = val), configPaths: ["settings.gesture.touch.yRatio"] },
                    { id: "gestureTouchAxesRatio", label: "Axis dominance", widget: "range", getValue: () => String(plug.config.touch.axesRatio), getRange: () => ({ min: 0.1, max: 10, step: 0.25, formatTooltip: (v: number) => v.toFixed(1) }), onChange: (val: number) => (plug.config.touch.axesRatio = val), configPaths: ["settings.gesture.touch.axesRatio"], getTipHTML: () => "Multiplier for vertical vs horizontal swipe dominance" },
                    { id: "gestureTouchThreshold", label: "Hold duration", widget: "input", inputs: [{ label: "ms", placeholder: "200", helperText: { info: "How long before a touch starts a swipe gesture, provided it did not move during the hold" }, type: "number", min: "0", value: () => plug.config.touch.threshold }], getValue: () => formatUITime(plug.config.touch.threshold), onChange: (val: Record<string, any>) => (plug.config.touch.threshold = val["ms"]), configPaths: ["settings.gesture.touch.threshold"] },

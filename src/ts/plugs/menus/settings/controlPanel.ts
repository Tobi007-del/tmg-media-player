...
import { CONTROLS, ROWS_ARR } from "@plugs/settings/controlPanel/build";
import type { AnyControl, ControlPanelBottomTuple } from "@plugs/settings/controlPanel/types";
import { getUIOpt, insertPanelCtrl, getPanelLocation } from "@utils/obj";
  ...
  items: [
    {
      ...
      label: "Control panel",
      widget: "group",
      getValue: () => ((plug.config.top && plug.config.top.some((c) => c !== "spacer")) || (plug.config.center && plug.config.center.some((c) => c !== "spacer")) || (plug.config.bottom && ROWS_ARR.map((i) => (plug.config.bottom as ControlPanelBottomTuple)[i]).some((row) => row?.some((c) => c !== "spacer"))) ? "On" : "Off"),
      ...
      items: [
        {
          ...
          items: CONTROLS.filter((id) => id !== "spacer")
            ...
            .map((id) => ({ id: `toggle-cp-${id}`, label: capitalize(uncamelize(id)), widget: "toggle" as const, getValue: () => (isEnabled(plug, id) ? "On" : "Off"), onChange: (val: boolean) => setEnabled(plug, id, val), configPaths: ["settings.controlPanel.top", "settings.controlPanel.center", "settings.controlPanel.bottom"] })),
        },
        {
          id: "layoutTimeline",
          label: "Timeline",
          widget: "group",
          getValue: () => (plug.config.timeline.disabled || plug.config.timeline.readonly ? "Off" : "On"),
          ...
          items: [
            ...
            { id: "timelineThumb", label: "Show thumb", widget: "select", getValue: () => getUIOpt(plug.config.timeline.thumb.options, plug.config.timeline.thumb.value), getOptions: () => plug.config.timeline.thumb.options!, onChange: (val: any) => (plug.config.timeline.thumb.value = val), configPaths: ["settings.controlPanel.timeline.thumb.value"] },
            { id: "timelinePreviews", label: "Previews", widget: "toggle", getValue: () => (plug.config.timeline.previews ? "On" : "Off"), onChange: (val: boolean) => (plug.config.timeline.previews = val), configPaths: ["settings.controlPanel.timeline.previews"], title: "Displays a thumbnail preview of the video frame when hovering over the timeline. Note: If a specific preview type (e.g. sprite/image src) was set by the developer, toggling this will reset it to 'auto'." },
                    ...
                    },
                    {
                      ...
                      items: [
                        ...
                        },
                        {
                          ...
                          getValue: () => formatMenuMs(plug.config.timeline.scrub.cancel.timeout),
            ...
            },
            {
              ...
              items: [
                ...
                },
                {
                  ...
                  label: "Markers",
                  widget: "drag-select",
                  getValue: () => `${plug.config.timeline.marks?.length || 0} marker${(plug.config.timeline.marks?.length || 0) !== 1 ? "s" : ""}`,
                  ...
                  onEdit: (idx: number) => (((plug as any)._editMarkerIdx = idx), plug.ctlr.plug("settings.settingsView")?.menu.goTo("timeline-marker-edit")),
                  ...
                  items: [
                    {
                      ...
                      widget: "input",
                      inputs: [
                        { name: "label", label: "Label", placeholder: "Chapter 2", helperText: { info: "e.g. The Plight of Kosi's Lover" }, required: true },
                        { name: "pos", label: "Position (%)", placeholder: "50", helperText: { info: "0 – 100" }, type: "number", required: true },
                        { name: "end", label: "End (%)", helperText: { info: "0 – 100 (Optional)" }, type: "number" },
                      ],
                      getValue: () => "",
                      onChange: (val: Record<string, string>) => (plug.config.timeline.marks ??= []).push({ label: val.label || undefined, start: Number(val.pos) || 0, end: val.end ? Number(val.end) : undefined }),
                    },
                    {
                      ...
                      widget: "input",
                      inputs: [
                        { name: "label", label: "Label", placeholder: "Chapter 2", helperText: { info: "e.g. The Plight of Kosi's Lover" }, required: true, value: () => plug.config.timeline.marks?.[(plug as any)._editMarkerIdx]?.label || "" },
                        { name: "pos", label: "Position (%)", placeholder: "50", helperText: { info: "0 – 100" }, type: "number", required: true, value: () => plug.config.timeline.marks?.[(plug as any)._editMarkerIdx]?.start || "" },
                        { name: "end", label: "End (%)", helperText: { info: "0 – 100 (Optional)" }, type: "number", value: () => plug.config.timeline.marks?.[(plug as any)._editMarkerIdx]?.end || "" },
                      ],
                      getValue: () => "",
                      onChange: (val: Record<string, string>) => {
                        const m = plug.config.timeline.marks?.[(plug as any)._editMarkerIdx];
        ...
        { id: "layoutDraggable", label: "Drag & drop layout", widget: "toggle", getValue: () => (plug.config.draggable ? "On" : "Off"), onChange: (val: boolean) => (plug.config.draggable = val), configPaths: ["settings.controlPanel.draggable"] },
      ],
    },
  ],
});

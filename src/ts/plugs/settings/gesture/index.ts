...
export class GesturePlug extends BasePlug<GestureConfig, GestureState> {
  ...
  protected handleKeyFocusIn({ target }: KeyboardEvent, t = target as HTMLElement): void {
    (t?.dataset?.controlId ?? t?.parentElement?.dataset?.controlId) === this.focusSubjectId && t.blur();
  }

  protected handleHoverPointerActive(e: Event, { target, pointerType } = e as PointerEvent): void {
    (!pointerType || !IS_MOBILE) && this.ctlr.plug("settings.overlay")?.show();
    pointerType && (target as HTMLElement).closest(".tmg-media-side-controls-wrapper") && clearTimeout(this.ctlr.plug("settings.overlay")?.overlayDelayId ?? -1);
  }

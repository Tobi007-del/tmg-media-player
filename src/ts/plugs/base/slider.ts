...
export abstract class BaseSliderPlug<Config extends OptRange, State extends SliderState> extends BasePlug<Config, State> {
  ...
  protected handleMax(max: number): void {
    ...
    this.settings.css[`currentMax${capitalize(this.prefix)}Ratio`] = max / 100;
  }
  protected handleSliderState(value: number): void {
    ...
    if (this.config.max > 100)
      if (value <= 100) {
        ...
        CSS[`current${capped}SliderBoostPosition`] = CSS[`current${capped}SliderBoostPercent`] = 0;
      } else {
        CSS[`current${capped}SliderPosition`] = 1;
        CSS[`current${capped}SliderBoostPosition`] = (value - 100) / (this.config.max - 100);
        CSS[`current${capped}SliderBoostPercent`] = CSS[`current${capped}SliderPercent`];
      }
  ...
  public toggle(option?: "auto"): void {
    if (option === "auto" && this.useAptValue && !this.state.aptValue) this.state.aptValue = this.config.skip;

...
export class Meta extends BaseComponent<MetaConfig, ComponentState, HTMLDivElement> {
  ...
  public override create(): HTMLDivElement {
    ...
    this.ctlr.DOM.mediaProfile = this.profile = this.ctlr.syncImgLoadState(createEl("img", { alt: "Profile", className: "tmg-media-profile" }));
    this.ctlr.DOM.mediaTitle = this.title = createEl("a", { className: "tmg-media-title tmg-media-meta-text" });
    this.ctlr.DOM.mediaArtist = this.artist = createEl("a", { className: "tmg-media-artist tmg-media-meta-text" });
    // DOM Injection

...
export class CastButton extends BaseComponent<CastConfig, ComponentState, HTMLButtonElement> {
  ...
  public override wire(): void {
    // Features Gating
    this.media.on("features.cast", this.gate, { init: this.ctlr.flags.wired, signal: this.signal });
    // Event Listeners
    this.el.addEventListener("click", this.handleClick, { signal: this.signal });
    // Ctlr Media Listeners
    this.media.on("state.cast", this.syncARIA, { init: this.ctlr.flags.wired, signal: this.signal });
    // ---- Config --------
